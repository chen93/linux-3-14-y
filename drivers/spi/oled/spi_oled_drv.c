...
{
	gpio_set_value(spi_oled_dc_pin, val);
}
...
static int spi_oled_probe(struct spi_device *spi)
{
	...
	spi_oled_dev = spi;
	spi_oled_dc_pin = (int)dev_get_platdata(&spi->dev);

	ret = devm_gpio_request(&spi->dev, spi_oled_dc_pin, "OLED_DC");
	if (ret < 0)
		return ret;
	gpio_direction_output(spi_oled_dc_pin, 0);

#ifndef CONFIG_TQ2440_USE_SPI_GPIO
	ret = devm_gpio_request(&spi->dev, spi->chip_select, "OLED_CHIP_SELECT");
	if (ret < 0)
		return ret;
